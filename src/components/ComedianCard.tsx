  ...
  return (
    <div className="group bg-white border border-charcoal/10 overflow-hidden hover:border-charcoal/20 transition-all duration-300">
      ...
      <div className="p-4 sm:p-5">
        <h3 className="font-display font-semibold text-charcoal text-base sm:text-lg">{comedian.name}</h3>
        {comedian.bio && (
        ...
        {hasUpcomingShow && (
          <Link
            href="/events"
            className="block mt-3 text-center py-2.5 border border-gold text-gold text-xs font-semibold hover:bg-gold hover:text-white transition-colors active:scale-[0.98]"
          >
            Upcoming Shows
          </Link>
        )}
