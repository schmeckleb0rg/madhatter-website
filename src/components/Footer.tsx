import Link from "next/link";

export default function Footer() {
  return (
    <footer className="bg-charcoal mt-24">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-12">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-10">
          {/* Brand */}
          <div>
            <div className="flex items-center gap-2 mb-3">
              <span className="font-display text-xl font-semibold text-off-white">
                Mad Hatter
              </span>
            </div>
            <p className="text-sm text-muted-dark leading-relaxed">
              Chicago&apos;s premier comedy club. Laugh like you mean it.
            </p>
            {/* Social links */}
            <div className="mt-4 flex items-center gap-4">
              <a
                href="https://instagram.com/madhattercomedy"
                target="_blank"
                rel="noopener noreferrer"
                className="text-muted-dark hover:text-gold transition-colors"
                aria-label="Instagram"
              >
                <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 100 12.324 6.162 6.162 0 000-12.324zM12 16a4 4 0 110-8 4 4 0 010 8zm6.406-11.845a1.44 1.44 0 100 2.881 1.44 1.44 0 000-2.881z"/></svg>
              </a>
              <a
                href="https://tiktok.com/@madhattercomedy"
                target="_blank"
                rel="noopener noreferrer"
                className="text-muted-dark hover:text-gold transition-colors"
                aria-label="TikTok"
              >
                <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24"><path d="M19.59 6.69a4.83 4.83 0 01-3.77-4.25V2h-3.45v13.67a2.89 2.89 0 01-2.88 2.5 2.89 2.89 0 01-2.89-2.89 2.89 2.89 0 012.89-2.89c.28 0 .54.04.79.1v-3.5a6.37 6.37 0 00-.79-.05A6.34 6.34 0 003.15 15.2a6.34 6.34 0 006.34 6.34 6.34 6.34 0 006.34-6.34V8.73a8.19 8.19 0 004.76 1.52v-3.4a4.85 4.85 0 01-1-.16z"/></svg>
              </a>
              <a
                href="https://facebook.com/madhattercomedy"
                target="_blank"
                rel="noopener noreferrer"
                className="text-muted-dark hover:text-gold transition-colors"
                aria-label="Facebook"
              >
                <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>
              </a>
              <a
                href="https://youtube.com/@madhattercomedy"
                target="_blank"
                rel="noopener noreferrer"
                className="text-muted-dark hover:text-gold transition-colors"
                aria-label="YouTube"
              >
                <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24"><path d="M23.498 6.186a3.016 3.016 0 00-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 00.502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 002.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 002.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z"/></svg>
              </a>
            </div>
          </div>

          {/* Shows */}
          <div>
            <h3 className="font-mono text-xs font-medium text-muted-dark uppercase tracking-widest mb-4">
              Shows
            </h3>
            <ul className="space-y-2">
              {[
                { href: "/events", label: "Upcoming Events" },
                { href: "/comedians", label: "Comedians" },
                { href: "/past-events", label: "Past Shows" },
                { href: "/open-mic", label: "Open Mic" },
                { href: "/tickets", label: "Tickets" },
              ].map((link) => (
                <li key={link.href}>
                  <Link
                    href={link.href}
                    className="text-sm text-muted-dark hover:text-off-white transition-colors"
                  >
                    {link.label}
                  </Link>
                </li>
              ))}
            </ul>
          </div>

          {/* More */}
          <div>
            <h3 className="font-mono text-xs font-medium text-muted-dark uppercase tracking-widest mb-4">
              More
            </h3>
            <ul className="space-y-2">
              {[
                { href: "/classes", label: "Comedy Classes" },
                { href: "/merch", label: "Merch" },
                { href: "/press", label: "Media & Press" },
                { href: "/visitor-info", label: "Visitor Info" },
                { href: "/about", label: "About" },
              ].map((link) => (
                <li key={link.href}>
                  <Link
                    href={link.href}
                    className="text-sm text-muted-dark hover:text-off-white transition-colors"
                  >
                    {link.label}
                  </Link>
                </li>
              ))}
            </ul>
          </div>

          {/* Contact */}
          <div>
            <h3 className="font-mono text-xs font-medium text-muted-dark uppercase tracking-widest mb-4">
              Find Us
            </h3>
            <address className="not-italic text-sm text-muted-dark space-y-1">
              <p>123 W Madison St</p>
              <p>Chicago, IL 60602</p>
              <p className="mt-3">
                <a
                  href="mailto:hello@madhattercomedy.com"
                  className="hover:text-off-white transition-colors"
                >
                  hello@madhattercomedy.com
                </a>
              </p>
              <p className="mt-2">
                <Link
                  href="/contact"
                  className="text-gold hover:underline"
                >
                  Contact Us
                </Link>
              </p>
            </address>
          </div>
        </div>

        <div className="mt-10 pt-6 border-t border-off-white/10 flex flex-col sm:flex-row items-center justify-between gap-4">
          <p className="text-xs text-muted-dark">
            &copy; {new Date().getFullYear()} Mad Hatter Comedy Club. All rights reserved.
          </p>
          <Link
            href="/admin/login"
            className="text-xs text-muted-dark/50 hover:text-muted-dark transition-colors"
          >
            Admin
          </Link>
        </div>
      </div>
    </footer>
  );
}
