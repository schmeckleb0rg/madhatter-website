...
export async function POST(request: Request) {
  ...
  if (!["favicon", "background", "og_image"].includes(type)) return NextResponse.json({ error: "Invalid type" }, { status: 400 });
  ...
  const settingKey = type === "favicon" ? "favicon_url" : type === "og_image" ? "og_image_url" : "background_url";
